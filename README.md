...
`frontend/` に Next.js の `node_modules` があるため、バックエンドのテスト対象を明示しています。
...
現在の実装では、`frontend/` に Next.js のフロントエンドを追加しています。
...
`frontend/.env.local` で Go API のURLを指定します。
